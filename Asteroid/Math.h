...
public:
	...
	static Matrix4 Lerp(Matrix4& a, Matrix4& b, float t) {
		Matrix4 temp;
		for (int i = 0; i < 4; i++) {
			for (int j = 0; j < 4; j++) {

				temp.mat[i][j] = (1 - t)*a.mat[i][j] + b.mat[i][j];
	...
	static Matrix4 createFromQuater(const class Quaternion& q);

	static Matrix4 createFromTrans(const Vector3& trans) {
		float temp[4][4] =
		{
...
public:
	...
	static Quaternion createFromMatrix(Matrix4& p) {
		Quaternion q = Quaternion();
		q.w = sqrt(Math::Max(0.0f, 1 + p.mat[0][0] + p.mat[1][1] + p.mat[2][2])) / 2;
		q.x = sqrt(Math::Max(0.0f, 1 + p.mat[0][0] - p.mat[1][1] - p.mat[2][2])) / 2;
		q.y = sqrt(Math::Max(0.0f, 1 - p.mat[0][0] + p.mat[1][1] - p.mat[2][2])) / 2;
		q.z = sqrt(Math::Max(0.0f, 1 - p.mat[0][0] - p.mat[1][1] + p.mat[2][2])) / 2;
		q.x *= Math::Sign(q.x * (p.mat[2][1] - p.mat[1][2]));
		q.y *= Math::Sign(q.y * (p.mat[0][2] - p.mat[2][0]));
		q.z *= Math::Sign(q.z * (p.mat[1][0] - p.mat[0][1]));
		return q;
